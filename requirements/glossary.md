...
| **TICK** | A server-side re-invocation of the PO turn loop, triggered every 15–30s while any non-clear team signal exists (peer inbox > 0, open PR, backlog > 0). Implemented as a `setTimeout` chain per thread in `src/lib/tick-scheduler.ts`. See US-026. |
| **AUTO-CONTINUE** | The synthetic message tag prepended to a tick invocation: `[[AUTO-CONTINUE: tick N | inflight=… | backlog=… | idle-peers=…]]`. Distinguishes server-scheduled ticks from user-initiated `talk_to_product_owner` pings. PO uses a terse dispatch-only mode on AUTO-CONTINUE; verbose reply mode on user pings. |
| **TICK_BUDGET** | Per-thread per-hour token/cost ceiling for autonomous ticking. Ticks halt when the budget is exceeded; they resume after the next clock-hour rollover. Default: 500K tokens/thread/hour (override: `APEX_TEAM_TICK_BUDGET_PER_HOUR=N`). A safety rail preventing unbounded background model spend. See US-026 AC6. |
| **NO_OP_THROTTLE** | Geometric backoff applied when PO emits zero `[[DISPATCH:]]` blocks on K consecutive ticks (K = 3). Each no-op doubles the tick delay; after K no-ops the scheduler pauses entirely. Resets on the next tick that emits ≥1 DISPATCH. Prevents runaway ticking when all signals are genuinely clear. See US-026 AC3. |
