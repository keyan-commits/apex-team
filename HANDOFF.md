...
| Subagent | Model | Size |
|---|---|---|
| `product-owner.md` | opus | 26 KB |
| `architect.md` | opus | 28 KB |
| `business-analyst.md` | sonnet | 26 KB |
| `ui-developer.md` | sonnet | 24 KB |
| `backend-developer.md` | sonnet | 25 KB |
| `qa.md` | sonnet | 47 KB |
| `devsecops.md` | sonnet | 28 KB |
| `ux-designer.md` | sonnet | 31 KB |

Each file = YAML frontmatter (`name`, `description`, `model`) + Plan C runtime adapter (translates apex-team's DISPATCH/HANDOFF/MCP semantics to subagent + files-on-disk semantics) + the original `role.systemPrompt` + the role's skill content. Generated by `scripts/port-subagents.ts` from `src/lib/roles.ts` `ROLES` — single source of truth maintained.
