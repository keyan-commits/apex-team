...
Every UX Designer spec that scopes to a single BA-defined feature MUST follow the FEAT-XXXX grouping convention. The convention applies in apex-team itself AND in any downstream workspace driven by the user-scoped subagents (LFM, bidshop, etc.). The five inline rules:

1. **Ticket prefix — `UX-XXXX`.** Your feature-scoped ticket prefix is `UX-XXXX` (zero-padded 4-digit, allocated monotonically by you, never reused). Pre-existing flat `design/<feature-slug>.md` specs remain valid; FEAT-scoped specs live in the new layout described below.

2. **Canonical artifact path.** UX Designer feature-scoped specs live at `design/features/FEAT-NNNN-<slug>/UX-NNNN-<slug>.md`. The per-feature subdirectory MUST be created only when there is at least one spec to deliver — empty placeholder directories are discouraged. For non-UI features (e.g. backend-only or pipeline-only work), no UX spec is required; the row in `requirements/features/INDEX.md` will show `0` in the UX column.

3. **Frontmatter rule.** Every deliverable file MUST open with a YAML frontmatter block containing at minimum `ticket: UX-NNNN`, `parent_feat: FEAT-NNNN`, `parent_us: US-NNN` (if applicable), `role: ux-designer`, and `status: <proposed|accepted|in-flight|done|superseded>`. The `parent_feat:` field is the primary cross-link — it is what the viewer uses to group artifacts by FEAT card and what `grep parent_feat: FEAT-XXXX` uses to compute count columns in `requirements/features/INDEX.md`.

4. **INDEX maintenance.** Allocate `UX` ticket numbers monotonically. Before a wave closes, add a row to `design/features/INDEX.md` with columns `Ticket | Parent FEAT | Parent US | Status | Description`. The UX `features/INDEX.md` is the allocation log for UX tickets — not a copy of the BA's `requirements/features/INDEX.md`.

5. **Cross-workspace applicability.** This convention applies in ANY workspace, not just apex-team. When invoked on a downstream project (LFM, bidshop, etc.), follow the same convention there — create the per-feature `design/features/FEAT-NNNN-<slug>/` directory in that project's design layout, link the `UX` deliverable to the BA's `FEAT-NNNN` allocation in that project, and maintain that project's `design/features/INDEX.md`.
